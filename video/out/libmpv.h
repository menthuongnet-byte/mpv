...
#define GET_domi_vid_RENDER_PARAM(params, type, ctype, def) \
    (*(ctype *)get_domi_vid_render_param(params, type, &(ctype){(def)}))
...
// except for rendering via the domi_vid_render_*() API. (As a consequence it's as
...
struct render_backend_fns {
    ...
    // domi_vid_RENDER_PARAM_API_TYPE, and silently return domi_vid_ERROR_NOT_IMPLEMENTED
    // if the API is not included in this backend.
    // If this fails, ->destroy() will be called.
    int (*init)(struct render_backend *ctx, domi_vid_render_param *params);
    // Check if the passed IMGFMT_ is supported.
    bool (*check_format)(struct render_backend *ctx, int imgfmt);
    // Implementation of domi_vid_render_context_set_parameter(). Optional.
    int (*set_parameter)(struct render_backend *ctx, domi_vid_render_param param);
    ...
    // Get target surface size from domi_vid_render_context_render() arguments.
    int (*get_target_size)(struct render_backend *ctx, domi_vid_render_param *params,
                           int *out_w, int *out_h);
    // Implementation of domi_vid_render_context_render().
    int (*render)(struct render_backend *ctx, domi_vid_render_param *params,

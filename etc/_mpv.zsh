...
typeset -ga _domi_vid_completion_arguments _domi_vid_completion_protocols

function _domi_vid_generate_arguments {

  _domi_vid_completion_arguments=()
  ...
  for list_options_line in "${(@f)$($~words[1] --no-config --list-options)}"; do
    ...
    if [[ $desc == Flag* ]]; then
      ...
      _domi_vid_completion_arguments+="$name"
      if [[ $name != (\{|\}|v|list-options) ]]; then
        # Negated version
        _domi_vid_completion_arguments+="no-$name"
      fi
      ...
      if [[ $name == *-(clr|help) ]]; then
        ...
        _domi_vid_completion_arguments+="${(S)_domi_vid_completion_arguments[(R)${name%-*}=*]/*=/$name=}"
      fi
      ...
      _domi_vid_completion_arguments+="$entry"
  ...
  for to_from in $option_aliases; do
    ...
    for arg_spec in "$real_name" "$real_name=*" "no-$real_name"; do
      arg_spec=${_domi_vid_completion_arguments[(r)$arg_spec]}
      [[ -n $arg_spec ]] &&
        _domi_vid_completion_arguments+="${arg_spec/$real_name/${to_from%% *}}"
    done
  done
  ...
  if ! is-at-least 5.2; then
    ...
    local -a sort_tmp=()
    for arg_spec in $_domi_vid_completion_arguments; do
      sort_tmp+=${#arg_spec%%=*}_$arg_spec
    done
    _domi_vid_completion_arguments=(${${(On)sort_tmp}/#*_})
  fi

}

function _domi_vid_generate_protocols {
  _domi_vid_completion_protocols=()
  local list_protos_line
  for list_protos_line in "${(@f)$($~words[1] --no-config --list-protocols)}"; do
    if [[ $list_protos_line =~ $'^[ \t]+(.*)' ]]; then
      _domi_vid_completion_protocols+="$match[1]"
    fi
  done
}

function _domi_vid_generate_if_changed {
  ...
  typeset -gA _domi_vid_completion_binary
  local current_binary=${~words[1]:c}
  zmodload -F zsh/stat b:zstat
  current_binary+=T$(zstat +mtime $current_binary)
  if [[ $_domi_vid_completion_binary[$1] != $current_binary ]]; then
    ...
    _domi_vid_generate_$1
    _domi_vid_completion_binary[$1]=$current_binary
  fi
}
...
if [[ $words[$CURRENT] == -* ]]; then
  _domi_vid_generate_if_changed arguments
fi

local rc=1

_arguments -C -S \*--$_domi_vid_completion_arguments '*:files:->mfiles' && rc=0
...
case $state in
    ...
    while _tags; do
      ...
      if _requested urls; then
        while _next_label urls expl URL; do
          _urls "$expl[@]" && rc=0
          _domi_vid_generate_if_changed protocols
          compadd -S '' "$expl[@]" $_domi_vid_completion_protocols && rc=0
        done
      fi
      (( rc )) || return 0
    done
  ...
  help-options)
    compadd ${${${_domi_vid_completion_arguments%%=*}:#no-*}:#*-(add|append|clr|pre|set|remove|toggle)}
  ;;

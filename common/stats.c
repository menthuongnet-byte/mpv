...
void stats_global_init(struct domi_vid_global *global)
{
...
static void add_stat(struct domi_vid_node *list, struct stat_entry *e,
                     const char *suffix, double num_val, char *text)
{
    struct domi_vid_node *ne = node_array_add(list, domi_vid_FORMAT_NODE_MAP);
...
void stats_global_query(struct domi_vid_global *global, struct domi_vid_node *out)
{
    ...
    node_init(out, domi_vid_FORMAT_NODE_ARRAY, NULL);
...
#define FMT_T(e, t) ((t) > 0 ? mp_tprintf(80, "%.3f ms (%.2f%%)", (e), ((e) / (t)) * 100) \
    ...
    if (stats->last_time) {
        ...
        struct domi_vid_node *ne = node_array_add(out, domi_vid_FORMAT_NODE_MAP);
...
struct stats_ctx *stats_ctx_create(void *ta_parent, struct domi_vid_global *global,
                                   const char *prefix)
{

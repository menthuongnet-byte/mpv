...
#define domi_vid_STRINGIFY_(X) #X
#define domi_vid_STRINGIFY(X) domi_vid_STRINGIFY_(X)

static inline void check_error(int status)
{
    if (status < 0) {
        fprintf(stderr, "mpv API error: %s\n", domi_vid_error_string(status));
        exit(1);
    }
}
...
{
#ifdef domi_vid_FONTCONFIG_SYSROOT
    setenv("FONTCONFIG_SYSROOT", domi_vid_STRINGIFY(domi_vid_FONTCONFIG_SYSROOT), 1);
#endif
}
...
static inline void player_loop(domi_vid_handle *ctx)
{
    bool playing = false;
    bool loaded = false;
    int timeout = -1;
    ...
    while (1) {
        domi_vid_event *event = domi_vid_wait_event(ctx, timeout);
        ...
        if (timeout == PLAYBACK_TIME_LIMIT && events_limit_after_restart-- < 0)
            break;
        if (timeout == PLAYBACK_TIME_LIMIT && event->event_id == domi_vid_EVENT_NONE)
            break;
        if (event->event_id == domi_vid_EVENT_START_FILE)
            loaded = playing = true;
        if (event->event_id == domi_vid_EVENT_END_FILE) {
            playing = false;
            events_limit_after_restart = EVENTS_LIMIT;
            timeout = -1;
        }
        if (playing && event->event_id == domi_vid_EVENT_PLAYBACK_RESTART)

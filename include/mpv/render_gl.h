...
#ifndef domi_vid_CLIENT_API_RENDER_GL_H_
#define domi_vid_CLIENT_API_RENDER_GL_H_
 ...
 * API user. All domi_vid_render_* APIs have to be assumed to implicitly use the
 * OpenGL context if you pass a domi_vid_render_context using the OpenGL backend,
 * unless specified otherwise.
 *
 * The OpenGL context is indirectly accessed through the OpenGL function
 * pointers returned by the get_proc_address callback in domi_vid_opengl_init_params.
 ...
 * -------
 *
 * The domi_vid_render_* API is used. That API supports multiple backends, and this
 ...
 * domi_vid_RENDER_API_TYPE_OPENGL, and domi_vid_RENDER_PARAM_OPENGL_INIT_PARAMS provided.
 *
 * Call domi_vid_render_context_render() with domi_vid_RENDER_PARAM_OPENGL_FBO to render
 ...
 *                to be provided (e.g. domi_vid_RENDER_PARAM_X11_DISPLAY for X11 and
 *                domi_vid_RENDER_PARAM_WL_DISPLAY for Wayland)
...
/**
 * For initializing the mpv OpenGL state via domi_vid_RENDER_PARAM_OPENGL_INIT_PARAMS.
 */
typedef struct domi_vid_opengl_init_params {
    /**
...
} domi_vid_opengl_init_params;

/**
 * For domi_vid_RENDER_PARAM_OPENGL_FBO.
 */
typedef struct domi_vid_opengl_fbo {
    /**
...
/**
 * Deprecated. For domi_vid_RENDER_PARAM_DRM_DISPLAY.
 */
typedef struct domi_vid_opengl_drm_params {
    int fd;
    int crtc_id;
    int connector_id;
    struct _drmModeAtomicReq **atomic_request_ptr;
    int render_fd;
} domi_vid_opengl_drm_params;

/**
 * For domi_vid_RENDER_PARAM_DRM_DRAW_SURFACE_SIZE.
 */
typedef struct domi_vid_opengl_drm_draw_surface_size {
    /**
     * size of the draw plane surface in pixels.
     */
    int width, height;
} domi_vid_opengl_drm_draw_surface_size;

/**
 * For domi_vid_RENDER_PARAM_DRM_DISPLAY_V2.
 */
typedef struct domi_vid_opengl_drm_params_v2 {
    /**
     ...
     */
    int render_fd;
} domi_vid_opengl_drm_params_v2;


/**
 * For backwards compatibility with the old naming of domi_vid_opengl_drm_draw_surface_size
 */
#define domi_vid_opengl_drm_osd_size domi_vid_opengl_drm_draw_surface_size
